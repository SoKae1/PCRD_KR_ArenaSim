    ...
    public partial class MainWindow : System.Windows.Window
    {
       ...
       // public static string[] AvailableCharaList = new string[MaxLoopCount];
        ...
        private void UnCheckAll()
        {
            ...
            tb_eriko_summer.IsChecked = false;
            tb_sizuru_summer.IsChecked = false;
            tb_nozomi_summer.IsChecked = false;
            tb_chika_summer.IsChecked = false;
            tb_tsumugi_summer.IsChecked = false;
                ...
                try
                {
                    ...
                    if (numCharacter == 5)
                    {
                        ...
                        for (int i = 0; i < 5; i++)
                        {
                            ...
                            if (temp[i] != "")
                                numCharaAbbr++;
                            temp2[i] = idnc.IDtoCharaKorName(idnc.CharaNameToID(temp[i]));
        ...
        */
                ...
                for (i = 15; i < 30; i++)//공격덱 행동
                {
                    ...
                    if (Battle_variable.death[i] == false)
                    {
                        ...
                        else if (state[i] == "C")//C 상태의 행동
                        {
                            ...
                            {
                                if (Battle_variable.name_eng[i] != "skull" && Battle_variable.name_eng[i] != "sylph_chika_1" && Battle_variable.name_eng[i] != "sylph_chika_2" && Battle_variable.name_eng[i] != "sylph_chika_christmas_1" && Battle_variable.name_eng[i] != "sylph_chika_christmas_2" && Battle_variable.name_eng[i] != "sylph_chika_christmas_3" && Battle_variable.name_eng[i] != "golem" && Battle_variable.name_eng[i] != "neneka_alter" && Battle_variable.name_eng[i] != "muimi_ub" && Battle_variable.name_eng[i] != "labyrista_ub")
                                {
                                    state[i] = "UB";
                                    UB_timing[i] = 0;
                                }
        ...
        public void ShowDefCharas(string[] defDeck_eng)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                uriS[i] = idnc.CharaNameToID(defDeck_eng[i]);
            }
        ...
        public void ShowOffCharas(string[] offDeck_eng)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                uriS[i] = idnc.CharaNameToID(offDeck_eng[i]);
            }
        ...
        public void ImagePrepare()
        {
            ...
            while (CharaIndex <= 174)
            {
                ...
                {
                    //6성
                    if (CharaIndex == 1 || CharaIndex == 2 || CharaIndex == 3 || CharaIndex == 4 || CharaIndex == 6 
                        || CharaIndex == 10 || CharaIndex == 11 || CharaIndex == 12 || CharaIndex == 16 || CharaIndex == 18 
                        || CharaIndex == 20 || CharaIndex == 21 || CharaIndex == 22 || CharaIndex == 23  || CharaIndex == 25 || CharaIndex == 28 || CharaIndex == 29
                        || CharaIndex == 30 || CharaIndex == 32 || CharaIndex == 33 || CharaIndex == 34 || CharaIndex == 36 
                        || CharaIndex == 40 || CharaIndex == 46 || CharaIndex == 48 || CharaIndex == 49 
                        || CharaIndex == 52 || CharaIndex == 58 || CharaIndex == 59
                        || CharaIndex == 60)
                    {
        ...
        public void searchIMG(Bitmap imageBig, int isOfforDef)
        {
            ...
            if (!WrongArea && isFoundFirstChara && numChara == 5)
            {
                ...
                for (int idx = 0; idx < 5; idx++)
                {
                    ...
                    Debug.WriteLine("ID " + Convert.ToString(idx) + " = " + DeckIndex[idx] + "," + idnc.IDtoCharaEngName(DeckIndex[idx], 6));

                    EngName[idx] = idnc.IDtoCharaEngName(DeckIndex[idx], 6);
                    KorName[idx] = idnc.IDtoCharaKorName(DeckIndex[idx], 6);
                }
                if (isOfforDef == 1)
        ...
        private void AutoReinStar6()
        {
            try
            {
                ...
                Debug.WriteLine(stringArray1[0]);
                ...
                while (!sr1.EndOfStream || !sr2.EndOfStream)
                {
                    ...
                    if (temp1[0] == "페코린느" || temp1[0] == "캬루" || temp1[0] == "콧코로" || temp1[0] == "리노" || temp1[0] == "리마" || temp1[0] == "이오"
                        ...
                        || temp1[0] == "노조미"
                        || temp1[0] == "스즈메" || temp1[0] == "쿠루미" || temp1[0] == "카오리" || temp1[0] == "아오이")
                    {
                        temp1[2] = "6";
                        temp2[2] = "6";
                    }
        ...
        private void AutoReinUniqueEquip(int isOfforDef)
        {
            try
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    string[] temp = s.Split(',');        // Split() 메서드를 이용하여 ',' 구분하여 잘라냄
                    if (temp[0] == "프페코" || temp[0] == "네네카" || temp[0] == "프유이" || temp[0] == "프코로" || temp[0] == "프캬루" || temp[0] == "라비리스타"
                        || temp[0] == "엔카리" || temp[0] == "엔요리" 
                        || temp[0] == "셰피" || temp[0] == "할무기" || temp[0] == "할레이" || temp[0] == "할츠리" || temp[0] == "마니카"
                        ...
                        || temp[0] == "신코토" || temp[0] == "성로에" || temp[0] == "성에루" 
                        || temp[0] == "시노리" || temp[0] == "타노리" || temp[0] == "시카야" || temp[0] == "타카야"
                        || temp[0] == "작오이" || temp[0] == "작마키" || temp[0] == "작후유" 
                        || temp[0] == "수치카" || temp[0] == "수리코" || temp[0] == "수즈루" || temp[0] == "수조미" || temp[0] == "수무기"
                        || temp[0] == "프레이")
                    {
                        temp[5] = "0";
                    }
                    ...
                    stringArray[row] = idnc.StringArrayToCommaString(temp);
                    row++;
        ...
        private void SetDeckWithAbbr(string s, int isOfforDef)
        {
            ...
            if (numCharacter == 5)
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    temp2[i] = idnc.IDtoCharaKorName(idnc.CharaNameToID(temp[i]),6);
                    Debug.WriteLine(string.Format("temp2[{0}] = {1}", i, temp2[i]));
                }
            ...
            if (args.Error == null)
            {
                if (args.IsUpdateAvailable)
                {
                    ...
                    if (dialogResult.Equals(System.Windows.Forms.DialogResult.Yes) || dialogResult.Equals(System.Windows.Forms.DialogResult.OK))
                    {
                        try
                        {
                            if (!File.Exists("NameConversion.dll"))
                            {
...
+ System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString() + @"/");
        ...
        private string AgilityTest()
        {
            ...
            HtmlNodeCollection divNodes =
                htmlDoc.DocumentNode.SelectNodes(@"//body/div[@class='application-main ']/div/main/turbo-frame/div/div[@class='clearfix container-xl px-3 px-md-4 px-lg-5 mt-4']/div/div[@class='Box']/div[@class='Box-body']");
            //                htmlDoc.DocumentNode.SelectNodes(@"//body/div[@class='application-main ']/div/main/turbo-frame/div/div[@class='clearfix container-xl px-3 px-md-4 px-lg-5 mt-4']/div/div[@class='Box']/div[@class='Box-body']");
